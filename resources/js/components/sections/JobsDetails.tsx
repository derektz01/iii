...
import { Calendar, FileText, User, File, Presentation, Video, Archive, Download, MapPin, Users } from "lucide-react";
...
export function JobsDetails({ job, language, onBack, onAttachmentClick }: JobsDetailsProps) {
  const translations = {
    en: {
      ...
      <Card>
        <CardHeader>
          <div className="flex items-start justify-between">
              ...
              <div className="space-y-2">
                {job.referenceCode && (
                  <p className="text-sm text-gray-600 font-mono">
                    {t.referenceCode}: {job.referenceCode}
                  </p>
                )}
                ...
                <div className="flex flex-wrap gap-4 text-sm text-gray-500">
                  {job.serialNumber && (
                    <div className="flex items-center gap-1">
                      ...
                      {t.serialNumber}: {job.serialNumber}
                    </div>
                  )}
                  ...
                  {(job.date || job.posted) && (
                    <div className="flex items-center gap-1">
                      ...
                      {t.publishDate}: {job.date || job.posted}
                    </div>
                  )}
                </div>
              </div>
            ...
            {job.issuer && (
              <Badge className={`${getIssuerColor(job.issuer)} ml-4`}>
                {job.issuer}
              </Badge>
            )}
          </div>
        ...
        <CardContent className="space-y-6">
          ...
          {job.content && (
            <div>
              ...
              <p className="text-gray-600 leading-relaxed">{job.content}</p>
            </div>
          )}
          
          {job.hasAttachment && job.attachments && job.attachments.length > 0 && (
            <div>
