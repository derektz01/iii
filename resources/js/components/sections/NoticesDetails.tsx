...
export function NoticesDetails({ notice, language, onBack, onAttachmentClick }: NoticesDetailsProps) {
  const translations = {
    en: {
      ...
      <Card>
        <CardHeader>
          <div className="flex items-start justify-between">
              ...
              <div className="space-y-2">
                {notice.referenceCode && (
                  <p className="text-sm text-gray-600 font-mono">
                    {t.referenceCode}: {notice.referenceCode}
                  </p>
                )}
                ...
                <div className="flex flex-wrap gap-4 text-sm text-gray-500">
                  {notice.serialNumber && (
                    <div className="flex items-center gap-1">
                      ...
                      {t.serialNumber}: {notice.serialNumber}
                    </div>
                  )}
            ...
            {notice.issuer && (
              <Badge className={`${getIssuerColor(notice.issuer)} ml-4`}>
                {notice.issuer}
              </Badge>
            )}
          </div>
        ...
        <CardContent className="space-y-6">
          ...
          {notice.hasAttachment && notice.attachments && notice.attachments.length > 0 && (
            <div>
